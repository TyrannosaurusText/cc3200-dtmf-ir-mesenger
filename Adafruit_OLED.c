...
void writeCommand(unsigned char c) {
    ...
    unsigned char* dummy = "";
    unsigned char* tx = &c;
    MAP_GPIOPinWrite(GPIOA0_BASE, 0x40, 0x00);

    MAP_GPIOPinWrite(GPIOA0_BASE, 0x80, 0x00);

    MAP_SPITransfer(GSPI_BASE,tx,dummy,1,SPI_CS_ENABLE|SPI_CS_DISABLE);
    MAP_GPIOPinWrite(GPIOA0_BASE, 0x40, 0x40); // OC Disable
...
void writeData(unsigned char c) {
    ...
    unsigned char* dummy = "";
    unsigned char* tx = &c;
    MAP_GPIOPinWrite(GPIOA0_BASE, 0x40, 0x0);
    MAP_GPIOPinWrite(GPIOA0_BASE, 0x80, 0x80);

    MAP_SPITransfer(GSPI_BASE,tx,dummy,1,SPI_CS_ENABLE|SPI_CS_DISABLE);

    MAP_GPIOPinWrite(GPIOA0_BASE, 0x40, 0x40);
...
*/
  ...
  GPIOPinWrite(GPIOA2_BASE, 0x40, 0x00);	// RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(GPIOA2_BASE, 0x40, 0x40);	// RESET = RESET_HIGH
